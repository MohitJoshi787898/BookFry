...
export interface BookDetailClientViewProps {
  initialBook: Book;
  slug: string;
}

export function BookDetailClientView({ initialBook, slug }: BookDetailClientViewProps) {
  const router = useRouter();
  const queryClient = useQueryClient();
  const { isAuthenticated } = useAuthStore();
  const { items, addItem } = useCartStore();

  const { data: book = initialBook } = useQuery<Book>({
    queryKey: ['book', slug],
  ...
  const cartItem = items.find(
    (it) => it.bookId === book.id || it.bookId === (book as { _id?: string })._id
  );
