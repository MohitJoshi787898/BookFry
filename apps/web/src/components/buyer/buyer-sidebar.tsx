...
const buyerNavGroups: NavGroup[] = [
  {
    ...
    items: [
      ...
      { name: 'Orders & Tracking', href: '/account/orders', icon: ShoppingBag, badge: 'Orders' },
      { name: 'Saved Wishlist', href: '/account/wishlist', icon: Heart, badge: 'Saved' },
    ],
  },
  {
    title: 'Community & Activity',
    items: [
      { name: 'Used Book Requests', href: '/account/requests', icon: MessageSquare, badge: 'P2P' },
      { name: 'Notifications', href: '/account/notifications', icon: Bell, badge: 'Alerts' },
    ],
  },
];

function BuyerQuickActionsCard() {
  const actions = [
    ...
    >
      <div className="relative flex h-full flex-col">
        ...
        <div className="flex-1 space-y-5 overflow-y-auto px-2.5 py-4">
          ...
          {!collapsed && <BuyerQuickActionsCard />}
        </div>

        {/* Footer Protected Badge & Storefront View */}
        <div className="border-t border-border/80 bg-background/40 px-2.5 py-3">
          ...
          >
            ...
            {!collapsed && <span>Marketplace Feed</span>}
          </Link>
        </div>
      </div>
    </aside>
