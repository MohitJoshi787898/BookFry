...
export function QuickFilterBar() {
  ...
  return (
    <div className="sticky top-14 sm:top-16 z-40 w-full bg-background/95 backdrop-blur-md border-b border-border/80 py-2.5 px-3 sm:px-6 lg:px-8 transition-colors font-sans shadow-2xs">
          ...
          <div className="relative">
            ...
            {activePopover === 'category' && (
              <div className="absolute top-full left-0 mt-2 w-52 p-3.5 bg-card border border-border rounded-2xl shadow-xl z-50 space-y-2">
                <div className="text-[10px] font-extrabold uppercase text-text-muted">Select Subject</div>
                {[
                  ...
                  <label key={c.val} className="flex items-center space-x-2 text-xs font-semibold cursor-pointer hover:text-secondary">
                    <input
                      type="radio"
                      name="category"
          ...
          <div className="relative">
            ...
            {activePopover === 'price' && (
              <div className="absolute top-full left-0 mt-2 w-48 p-3.5 bg-card border border-border rounded-2xl shadow-xl z-50 space-y-2">
                <div className="text-[10px] font-extrabold uppercase text-text-muted">Select Price Range</div>
                {[
                  { label: 'Under ₹200', val: '0-200' },
                  { label: '₹200 to ₹500', val: '200-500' },
                  { label: 'Over ₹500', val: '500-2000' },
                ].map((p) => (
                  <label key={p.val} className="flex items-center space-x-2 text-xs font-semibold cursor-pointer hover:text-secondary">
                    <input
                      type="radio"
                      name="price"
