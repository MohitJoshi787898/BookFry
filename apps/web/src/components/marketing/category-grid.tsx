...
export function CategoryGrid({
  ...
  return (
    <section aria-label={title} className="py-16 sm:py-20 bg-background font-sans border-b border-border transition-colors duration-200">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 space-y-10">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-6">
          {categoryList.map((cat, idx) => {
            ...
            const grad = cat.gradient || 'from-blue-500/10 to-indigo-500/10 text-brand dark:text-primary';
            ...
            return (
              ...
              >
                <Link
                  href={cat.href || '/books'}
                  className={`group relative flex flex-col justify-between p-5 sm:p-6 h-44 rounded-2xl border border-border bg-card hover:shadow-lg transition-all duration-300 ${bColor} overflow-hidden`}
                >
                  <div className="flex items-start justify-between">
                    <div className={`p-3 rounded-xl bg-gradient-to-br ${grad} group-hover:scale-110 transition-transform duration-300`}>
